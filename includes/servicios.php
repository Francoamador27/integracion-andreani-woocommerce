<tr valign="top" id="packing_options">
	...
	<?php 
            if ( ! defined( 'ABSPATH' ) ) exit; 
   				...
   				wp_register_style( 'services_css',ANDREANI_PLUGIN_URL. '/includes/css/servicios.css'  , ANDREANI_VERSION,  
				   true);
		...
		<table class="flexipaas_boxes widefat">
			...
			<tbody id="rates">
				<?php 	
				     if ( ! defined( 'ABSPATH' ) ) exit;  
					if ( $this->instance_settings['tipo_servicio'] ) {
 
						foreach ( $this->instance_settings['tipo_servicio'] as $key => $box ) {
 							if ( ! is_numeric( $key ) )
								continue;
							?>
							<tr>
								...
								<td><div class="tooltip"><input class="operativa" type="text" size="15" name="contrato_numero[<?php echo esc_attr($key); ?>]" value="<?php echo esc_attr( $box['contrato_numero'] ); ?>" /> <span class="tooltiptext">Contrato proporcionado por el contacto comercial de Andreani</span></div></td>
								<td><div class="tooltip"><input type="text" size="35" name="contrato_modalidad_desc[<?php echo esc_attr($key); ?>]" value="<?php echo esc_attr( $box['contrato_modalidad_desc'] ); ?>" />  <span class="tooltiptext">Agregar una descripción para identificar el servicio contratado</span></div></td>
						 
 
 								<td><input type="checkbox" name="modalidad_activa[<?php echo esc_attr($key); ?>]" <?php checked( ! isset( $box['modalidad_activa'] ) || $box['modalidad_activa'] == 1, true ); ?> /></td>
							</tr>
							<?php
						}
					}
		 ...
		 <?php 
    				wp_register_script( 'services', ANDREANI_PLUGIN_URL.'/includes/js/servicios.js', array('jquery'),  ANDREANI_VERSION,  
				   true);
